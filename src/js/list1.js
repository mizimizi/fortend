(function () {
    ...
    var ajaxUrl = window.parent.getApi()+'/org/standard/api/getBilling';
   ...
   var  _table = $table.dataTable($.extend(true,{}, {
    ...
    function searchFUN(type){
        var formval2 = $.trim($(".branchcode").val()),
        ...
        if(formval2||formval5){
            if(type){
                ...
                var hrefUrl = ajaxAPI+"/org/standard/api/getBilling/down?"+excelDate
                window.open(hrefUrl);
            }
        }else{
            layer.msg('请输入要查询的内容', {
