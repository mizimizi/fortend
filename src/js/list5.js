(function () {
    ...
    var ajaxUrl = window.parent.getApi()+'/org/standard/api/dangweiDetail';
        ...
        getQueryCondition :function(data) {
            var param = {};
                param.branchcode= $.trim($(".committeecode").val()),
                    param.branchcode= $.trim($(".themonthe").val()),
                    param.state= $.trim($(".state").val()),
                param.themonthe = optuser;
            //组装分页参数
   ...
   var  _table = $table.dataTable($.extend(true,{}, {
    ...
    function searchFUN(type){
        var formval2 = $.trim($(".committeecode").val()),
            formval3 = $.trim($(".themonthe").val()),
            formval5 = $.trim($(".state  option:selected").val());
        if(formval2||formval5||formval3){
                _table.draw();
        }else{
            layer.msg('请输入要查询的内容', {
                time: 1000,
